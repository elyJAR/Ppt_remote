 ...
 *
 * Uses only Java's built-in com.sun.net.httpserver — no extra dependency.
 */
class WebFileServer(
    ...
    fun start(): Boolean {
        ...
        for (attempt in 0..2) {
            try {
                httpServer = HttpServer.create(InetSocketAddress(boundPort + attempt), 0)
                boundPort += attempt
                break
            } catch (_: Exception) {}
        }
        ...
        httpServer.createContext("/", ::handleRoot)
        httpServer.createContext("/login", ::handleLogin)
        httpServer.createContext("/api/files", ::handleList)
        httpServer.createContext("/download", ::handleDownload)
        httpServer.createContext("/upload", ::handleUpload)
        httpServer.createContext("/delete", ::handleDelete)
        httpServer.start()
        server = httpServer
    ...
    private fun sessionValid(exchange: HttpExchange): Boolean {
        if (!isPinRequired()) return true
        val token = activeSessionToken ?: return false
        val cookieHeader = exchange.requestHeaders.getFirst("Cookie") ?: return false
    ...
    private fun handleRoot(exchange: HttpExchange) {
        ...
        val pathParam = exchange.requestURI.query
            ?.split("&")?.firstOrNull { it.startsWith("path=") }
            ?.removePrefix("path=")
        val dir = if (pathParam != null) safeResolve(pathParam) else File(rootPath).canonicalFile
        ...
        when (exchange.requestMethod.uppercase()) {
            ...
            "POST" -> {
                ...
                if (submitted == pin) {
                    ...
                    exchange.responseHeaders.add("Set-Cookie", "session=$token; Path=/; HttpOnly")
    ...
    private fun handleUpload(exchange: HttpExchange) {
        ...
        try {
            val bufferedInput = java.io.BufferedInputStream(exchange.requestBody)
            val savedName = parseMultipartAndSave(bufferedInput, boundary, dir)
            if (savedName != null) {
    ...
    private fun parseMultipartAndSave(input: java.io.InputStream, boundary: String, dir: File): String? {
        ...
        try {
            ...
            while (true) {
                // Check if current window matches standard boundary separator
                if (windowLen == window.size && window.contentEquals(separator)) {
                    break
                }
                
                // Also check if current window starts with the end boundary directly
                val endSep = "--$boundary--".toByteArray(StandardCharsets.ISO_8859_1)
                if (windowLen >= endSep.size && window.sliceArray(0 until endSep.size).contentEquals(endSep)) {
                    break
                }
    ...
    private fun sendRedirect(exchange: HttpExchange, location: String) {
    ...
    private fun sendHtml(exchange: HttpExchange, status: Int, html: String) {
